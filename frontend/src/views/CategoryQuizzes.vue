...
/* Сетка квизов - ОСНОВНЫЕ ИЗМЕНЕНИЯ */
.quizzes-grid {
  display: grid;
  ...
  margin-bottom: 40px;
  width: 100%;
  align-items: stretch; /* Важно: растягиваем все элементы */
}

/* Изменение размеров карточек для лучшего отображения в строке */
.quiz-card {
  background: white;
  border-radius: 15px;
  overflow: hidden;
  ...
  display: flex;
  flex-direction: column;
  height: 100%; /* Важно: занимает всю высоту ячейки */
  min-height: 420px; /* Фиксированная минимальная высота */
}
...
.quiz-content {
  ...
  display: flex;
  flex-direction: column;
  min-height: 220px; /* Минимальная высота контента */
}
...
.quiz-title {
  ...
  color: #1f2937;
  line-height: 1.3;
  min-height: 2.4rem; /* Фиксированная высота заголовка */
  overflow: hidden;
...
.quiz-description {
  ...
  overflow: hidden;
  display: -webkit-box;
  -webkit-line-clamp: 3; /* Фиксированное количество строк */
  -webkit-box-orient: vertical;
}
...
@media (max-width: 768px) {
  ...
  .quizzes-grid {
    grid-template-columns: 1fr; /* 1 карточка в строке на мобильных */
  }
